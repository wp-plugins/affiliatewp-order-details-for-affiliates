		...
		switch ( $referral->context ) {
			...
			case 'edd':
				...
				if ( $info == 'customer_shipping_address' ) {
					...
					if ( $is_allowed['customer_shipping_address'] && ! empty( $address ) ) {
						$customer_address = ! empty ( $address['address'] ) ? $address['address'] . '<br />' : '';
						$customer_address .= ! empty ( $address['address2'] ) ? $address['address2'] . '<br />' : '';
						$customer_address .= ! empty ( $address['city'] ) ? $address['city'] . '<br />' : '';
						$customer_address .= ! empty ( $address['zip'] ) ? $address['zip'] . '<br />' : '';
						$customer_address .= ! empty ( $address['state'] ) ? $address['state'] . '<br />' : '';
						$customer_address .= ! empty ( $address['country'] ) ? $address['country'] . '<br />' : '';
					}
				...
				if ( $info == 'customer_billing_address' ) {
					...
					if ( $is_allowed['customer_billing_address'] && ! empty( $address ) ) {
						$customer_address = ! empty ( $address['line1'] ) ? $address['line1'] . '<br />' : '';
						$customer_address .= ! empty ( $address['line2'] ) ? $address['line2'] . '<br />' : '';
						$customer_address .= ! empty ( $address['city'] ) ? $address['city'] . '<br />' : '';
						$customer_address .= ! empty ( $address['zip'] ) ? $address['zip'] . '<br />' : '';
						$customer_address .= ! empty ( $address['state'] ) ? $address['state'] . '<br />' : '';
						$customer_address .= ! empty ( $address['country'] ) ? $address['country'] . '<br />' : '';
